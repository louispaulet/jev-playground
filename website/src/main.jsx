...
function Footer() {
  return <footer className="mt-auto border-t border-white/10 bg-ink px-5 py-6 text-xs text-slate-500 lg:px-8">
    <div className="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <span>Built for inspection, not mystique.</span>
    </div>
  </footer>
}
...
function GenderPage() {
  ...
  return <Shell>
    <ReportHeader eyebrow="Experiment 01 / TypeSafe · JEV Choice" title="A surname is not a person. It is still a useful test." description="This report runs 2,215 of 2,239 JEV classifications from the gender-guesser Python library, then leaves the slices and failure modes visible." >
      <div className="rounded-2xl border border-acid/20 bg-acid/5 p-5"><div className="font-mono text-xs uppercase tracking-widest text-acid">run stamp</div><div className="mt-3 font-display text-2xl font-semibold text-white">seed 20260925</div><div className="mt-2 text-sm leading-6 text-slate-400">24 rows excluded because geo support was below 400. No benchmark requests were made while regenerating the report.</div></div>
    </ReportHeader>
    <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Stat value="2,215" label="evaluated classifications" /><Stat value="94.5%" label="male precision" accent="teal" /><Stat value="55.5%" label="female recall" accent="coral" /><Stat value="24" label="excluded rows" /></div></section>
    <section className="mx-auto max-w-7xl px-5 pb-16 lg:px-8"><SectionTitle kicker="01 / summary" title="Precision and recall by gender" note="Ground truth is the normalized gender-guesser label; andy becomes unisex." /><DataTable headers={['Gender', 'Support', 'Precision', 'Recall', 'True positives', 'False positives', 'False negatives']} rows={[['Male', '1,103', '94.5%', '74.3%', '820', '48', '283'], ['Female', '1,112', '99.0%', '55.5%', '617', '6', '495'], ['Unisex', '0', '0.0%', '0.0%', '0', '724', '0']]} renderRow={(row, index) => <tr key={row[0]} className="text-slate-300"><td className="px-4 py-4 font-semibold text-white">{row[0]}</td>{row.slice(1).map((cell, i) => <td key={i} className={`px-4 py-4 ${i === 1 ? 'text-teal' : ''}`}>{cell}</td>)}</tr>} /></section>
    <section className="mx-auto max-w-7xl px-5 pb-16 lg:px-8"><SectionTitle kicker="02 / slices" title="Where the signal changes" note="Regions come from country-specific gender-guesser signals, not claims about a person’s origin." /><DataTable headers={['Region', 'Support', 'Accuracy', 'Macro precision', 'Macro recall', 'Correct']} rows={regions} renderRow={(row) => <tr key={row[0]} className="text-slate-300"><td className="px-4 py-4 font-semibold text-white">{row[0]}</td>{row.slice(1).map((cell, i) => <td key={i} className={`px-4 py-4 ${i === 1 ? 'text-acid' : ''}`}>{cell}</td>)}</tr>} /></section>
    ...
    <section className="border-y border-white/10 bg-panel/50"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 lg:grid-cols-[1fr_1fr] lg:px-8"><div><Kicker>Read the result carefully</Kicker><p className="max-w-xl text-base leading-7 text-slate-300">The model is better at avoiding false gender labels than it is at recovering every expected label. That makes the unisex prediction a useful uncertainty signal, not a defect to hide.</p></div><p className="text-sm leading-7 text-slate-500">The source report notes that male/female include gender-guesser’s mostly_male/mostly_female labels. Country signals describe the library’s signal coverage; they do not establish a person’s gender or geographic origin.</p></div></section>
  </Shell>
}
...
function WikipediaPage() {
  return <Shell>
    ...
    <section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8"><SectionTitle kicker="01 / found routes" title="Start at the left. Target at the right." note="The route is the successful branch, not every candidate explored by the beam search." /><div className="space-y-4">{routes.map((route, index) => <RouteCard key={route.label} route={route} index={index} />)}</div></section>
