...
function GenderPage() {
  ...
  return <Shell>
    ...
    <section className="mx-auto max-w-7xl px-5 pb-16 lg:px-8"><SectionTitle kicker="02 / slices" title="Performance changes by geo slice" note="Asia is the low point at 46.5%; Middle East and mixed/ambiguous are highest at 73.0%. These are dataset segments, not claims about people." /><DataTable headers={['Region', 'Support', 'Accuracy', 'Macro precision', 'Macro recall', 'Correct']} rows={regions} renderRow={(row) => <tr key={row[0]} className="text-slate-300"><td className="px-4 py-4 font-semibold text-white">{row[0]}</td>{row.slice(1).map((cell, i) => <td key={i} className={`px-4 py-4 ${i === 1 ? 'text-acid' : ''}`}>{cell}</td>)}</tr>} /></section>
    ...
    <section className="border-y border-white/10 bg-panel/50"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 lg:grid-cols-[1fr_1fr] lg:px-8"><div><Kicker>Read the result carefully</Kicker><p className="max-w-xl text-base leading-7 text-slate-300">The most interesting behavior is not “male precision versus female recall.” It is that JEV uses <span className="text-teal">unisex</span> as an escape hatch: 724 predictions land there, while this benchmark has no unisex ground-truth rows. The confidence gap suggests that the model is at least somewhat more certain when it is correct.</p></div><p className="text-sm leading-7 text-slate-500">The source report notes that male/female include gender-guesser’s mostly_male/mostly_female labels. Country signals describe the library’s signal coverage; they do not establish a person’s gender or geographic origin.</p></div></section>
  </Shell>
}
